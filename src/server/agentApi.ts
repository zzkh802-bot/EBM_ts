...
export function buildAgentPrompt(input: AgentRunInput, attachmentContext = ""): string {
  ...
  return [
    ...
    "证据边界选择：start_text 和 end_text 都要从当前 read 原文中复制，优先各取包含章节/小节上下文、主张关键词以及数字或单位的连续片段（通常 12–40 个字符以上），让整对边界在来源中唯一。不要只用‘结果’、‘结论’、‘推荐’或‘(证据等级 2b)’等重复短语；如果工具提示有多个候选，先加长边界或缩小读取行段，再重试 evidence_add。",

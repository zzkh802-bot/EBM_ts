...
export function registerEbmTools(pi: ExtensionAPI): void {
  ...
  pi.registerTool({
    ...
    promptGuidelines: [
      "Use evidence_add only after reading the archived source. Choose exactly one locator mode: (1) read_id plus start_text and end_text copied from that read; source_path is optional because the receipt carries it, and line_start/line_end may optionally narrow the location to a subrange inside that read; or (2) source_path plus line_start and line_end, with text anchors optional.",

...
export function registerEbmTools(pi: ExtensionAPI): void {
  ...
  pi.registerTool({
    ...
    promptGuidelines: [
      "Use evidence_add only after reading the archived source. Choose one locator mode: (1) read_id plus start_text and end_text copied from that read; source_path is optional and line_start/line_end are optional absolute-source-line narrowing hints; or (2) source_path plus line_start and line_end, with optional start_text/end_text. A read_id still requires both text anchors; line-range mode does not. When the read receipt provides absolute lines, prefer adding the matching line_start/line_end to narrow repeated phrases; if line numbers came from another candidate or read, omit them rather than combining them with this read_id. Never convert absolute source lines into read-window-relative offsets.",

...
export function registerWebTools(pi: Pick<ExtensionAPI, "registerTool" | "events">): void {
  pi.registerTool({
    ...
    promptGuidelines: ["This is a persistent cross-session index. Search it first for each new sub-question; if a direct hit has source_url, call web_read on that URL to materialize the cached source in the current session. For mcp:// source_url values, web_read performs the local-library read; never use an imported_from path or a historical session path with native read. Do not treat the result list or snippet as evidence. Use Chinese queries for Chinese guideline/library content.", "guideline_mcp_search returns document candidates only; use guideline_mcp_read for the needed source context. guideline_mcp_retrieve is temporarily disabled."],
  ...
  pi.registerTool({
    ...
    promptGuidelines: ["After reading, choose read_id with matching absolute-source line_start/line_end (source_path optional; never copy line hints from another candidate/read). For a tight decision-relevant passage of no more than 12 source lines, anchors may be omitted; otherwise use the shortest distinctive continuous start_text/end_text (semantic completeness is unnecessary; include a nearby local word when a marker repeats). Or use source_path with line_start/line_end. For an internal mcp:// source_url, call web_read to materialize the local-library copy in the current session; never read an imported_from historical filesystem path. Layout/XML/entity/punctuation noise is normalized; if read_id anchors mismatch, use the known tight range or choose more distinctive boundaries/reread a narrower window rather than archiving a broad whole read. Never paraphrase, repair clinical text, or join discontinuous passages."],
    parameters: Type.Object({

...
Focused retrieval also returns opaque `source_span_id` values bound to `source_id`, exact character coordinates, and a quote digest. Passing a span ID to `evidence_add` archives the already-selected continuous source text without requiring the model to copy or relocalize it. This is the preferred path for MCP chunks. A copied `quote` with `source_id` remains available for full documents and PubMed abstracts. The evidence tool first requires a unique exact match, then permits only presentation normalization such as line wrapping, Unicode spacing, zero-width characters, and paired Markdown emphasis markers. The canonical archived quote, including its original Markdown, is always stored. Changed numbers, drug names, wording, clinical punctuation, OCR substitutions, translations, reconstructed table prose, and discontinuous ellipses are never auto-accepted.

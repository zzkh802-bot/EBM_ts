...
- A live quick-mode run exhausted its former five-iteration budget before report finalization. Raised the quick-mode baseline to eight iterations (ten when deep thinking is enabled), and aligned the API fallback so the formal-report contract has a realistic minimum budget.

...
- Made the formal report the primary completion surface: when the service returns a report it is rendered directly in the completed conversation, with a workspace read-back fallback when only a report path is returned. The file tree and report library now support review instead of hiding the delivered result behind another click.

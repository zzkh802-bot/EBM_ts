...
4. Add evidence only from a minimal, sufficient, continuous verbatim source passage using `evidence_add`; pass the copied passage as `quote` and let the tool derive source coordinates. Do not calculate or guess line ranges. If location fails, copy a canonical candidate returned by the tool and retry; candidates are not evidence until registration succeeds. Never repair changed numbers, drug names, wording, OCR characters, translate the source, or join discontinuous passages with ellipses. Treat evidence records as claim-linked evidence candidates, not final conclusions: use `confidence=low` or `moderate` when directness or interpretation is still uncertain, and reserve `high` for directly applicable, clearly interpreted evidence. For PubMed abstracts, quote only the archived `Abstract` section and never cite `PubMed context`. PDF tables from guideline MCP may be linearized out of column order: do not register visibly fragmented table text. Prefer a nearby narrative sentence that states the same result; use a table only when its row-column structure remains unambiguous.

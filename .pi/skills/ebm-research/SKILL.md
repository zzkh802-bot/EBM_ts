...
2. For guideline-answerable clinical recommendation questions, use MCP before broad biomedical search. Prefer `guideline_mcp_retrieve` for focused chunks and read a directly supporting source path before registering its passage with `read_id` and exact start/end text; if you need document identity, date, recommendation context, applicability boundaries, or conflict resolution, use `guideline_mcp_search`/`guideline_mcp_read`. After the first useful MCP result, use `research_frame_scratchpad_append` only if the MCP result changes the retrieval strategy, exposes a freshness gap, or creates a clear stop condition. If 1-2 traceable chunks directly answer a sub-question, add evidence from those passages and stop expanding that sub-question unless there is conflict, missing context, old guidance, or a fast-moving topic.
